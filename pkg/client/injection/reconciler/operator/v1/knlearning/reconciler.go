...
import (
	...
	operatorv1 "knstart/pkg/client/listers/operator/v1"
	reflect "reflect"
...
// controller reconciling v1.KNLearning.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v1.KNLearning. Any changes
	// to the objects .Status or .Finalizers will be propagated to the stored
	// object. It is recommended that implementors do not call any update calls
	// for the Kind inside of ReconcileKind, it is the responsibility of the calling
	// controller to propagate those properties. The resource passed to ReconcileKind
	// will always have an empty deletion timestamp.
	ReconcileKind(ctx context.Context, o *v1.KNLearning) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1.KNLearning.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v1.KNLearning. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v1.KNLearning) reconciler.Event
}

// ReadOnlyInterface defines the strongly typed interfaces to be implemented by a
// controller reconciling v1.KNLearning if they want to process resources for which
// they are not the leader.
type ReadOnlyInterface interface {
	// ObserveKind implements logic to observe v1.KNLearning.
	// This method should not write to the API.
	ObserveKind(ctx context.Context, o *v1.KNLearning) reconciler.Event
}

// ReadOnlyFinalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1.KNLearning if they want to process tombstoned resources
...
type ReadOnlyFinalizer interface {
	// ObserveFinalizeKind implements custom logic to observe the final state of v1.KNLearning.
	// This method should not write to the API.
	ObserveFinalizeKind(ctx context.Context, o *v1.KNLearning) reconciler.Event
}

// reconcilerImpl implements controller.Reconciler for v1.KNLearning resources.
type reconcilerImpl struct {
	...
	Lister operatorv1.KNLearningLister
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client versioned.Interface, lister operatorv1.KNLearningLister, recorder record.EventRecorder, r Interface, options ...controller.Options) controller.Reconciler {
...
func (r *reconcilerImpl) Reconcile(ctx context.Context, key string) error {
	...
	getter := r.Lister.KNLearnings(namespace)
...
func (r *reconcilerImpl) updateStatus(existing *v1.KNLearning, desired *v1.KNLearning) error {
	existing = existing.DeepCopy()
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
		if attempts > 0 {

			getter := r.Client.Xq2005V1().KNLearnings(desired.Namespace)
		...
		updater := r.Client.Xq2005V1().KNLearnings(existing.Namespace)
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1.KNLearning) (*v1.KNLearning, error) {

	getter := r.Lister.KNLearnings(resource.Namespace)
	...
	patcher := r.Client.Xq2005V1().KNLearnings(resource.Namespace)
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1.KNLearning) (*v1.KNLearning, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1.KNLearning, reconcileEvent reconciler.Event) (*v1.KNLearning, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
